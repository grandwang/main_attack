...
from attackface.Utilities_face import *
...
class Swarm:
    ...
    # N,model,targetLabel,maxChange,dataset,dLength,verbosity,topN,targeted,queries
    ...
    def reshapeAndCompare(self, positions):
        z = re2Watermark(positions, self.dataset, self.img, self.watermark, self.sl, self.numofMethods,
                         self.numberOfParticles, self.input_size)  # sample,dataset,img,watermark,sl
    ...
    def initX(self, net, xs, label, input, classes, watermark, sl):
        ...
        _, fstconfi = cos_main(net, img, '', classes)
    ...
    def generateISSIndicesDictionary(self):
        self.issIndices = {}
        cd = list(product([0, -1, 1], repeat=self.channels))  # product(A, B)函数，返回A、B中的元素的笛卡尔积的元组
        if self.channels == 3:  # 返回的是数组中，非零元素的位置，cd为只保留仅存在一个非零元素的元组
    ...
    def generateIndividualSearchSpaces(self):
        ...
        # self.granularityCurrentLevel初始为0,起查询索引作用
        self.ISS = splitSoluIntoNbyNRegions(self.numofMethods, self.numberOfParticles, 3,
                                            self.granularityLevels[self.granularityCurrentLevel])  # (3,50,3,5)
    ...
    def updateChangeRate(self):
        ...
        # self.changeRate = int(np.ceil(np.random.uniform(0,lenOfIndices,1)))
        self.changeRate = lenOfIndices

    def initializeSwarm(self, sample):
        self.Particles = [particle] * self.numberOfParticles  # len=5 <class 'particle.particle'>
    ...
    def initializeSwarmAndParticles(self, solu_plc, fitness, net, input, classes, j):
        ...
        self.flag = False
        if self.targeted == True:  # 指向与非指向
            self.swarmLabel = -1
        else:
            self.swarmLabel = self.classIND  # classIND==ind==targetlabel==getTargetLabel(pred,i)
    ...
    def initializeParticles(self, startingPosition, net, input, classes, j):
        ...
        for particles in range(0, self.numberOfParticles):
            ...
            p.currentVelocity = self.set_init_velocity()  # dLength == N x 3 == 150
            # 存在初始化particle时，就成功的情况
    ...
    def fitScore(self, V, r1, r2, sigema_max, sigema_min, net, input, classes, j):
        ...
        for i in range(len(V)):
            # r1 : share the same shape with V
            if r1[i] < sigfun(V[i], sigema):  # 个体
                self.P[i] = self.currentPosition[i] + V[i]
            else:
                self.P[i] = self.currentPosition[i]
            if r2[i] > sigfun_quo(V[i], sigema):  # 群体
                self.P_quo[i] = self.currentPosition[i] + V[i]
            else:
                self.P_quo[i] = self.currentPosition[i]
        P = self.P.reshape(self.numofMethods * self.numberOfParticles, 3)  # (80,3)
        ...
        self.fit, idx = fitnessScore(self, net, P, input, classes, self.watermark, self.sl,
                                     j)  # (numofMethods * numOfParticles, 1)
        ...
        if self.fit == True:
            ...
            i = 0
            for elem, elem_quo in zip(self.fit, self.fit_quo):  # the smaller, the better
                if elem < self.fitP[i]:
                    self.fitP[i] = elem
                ...
                if self.fitP[i] < self.fitG:
                    ...
                    self.swarmBestPosition = self.bestPosition[i]  # 确定swarmBestPosition.shape
    ...
    def moveParticleAndCalNewFitness(self, p, net, input, classes, j):
        ...
        # p.updateNextPostion(self.bestPosition,self.maxChangeLower, self.maxChangeUpper)
        p.setCurrentPosition(self.currentPosition)
        # swarm.fitP -> particle.currentposition
    ...
    def randomizeParticle(self, startingPosition, p, net, input, classes, j):
        ...
        newFitness, newProba = self.InitialFitScore(p.currentVelocity, r1_p, r2_p, self.sigema_max, self.sigema_min,
                                                    net, input, classes, j)
        # p.currentFitness = np.zeros(self.numofMethods*self.numberOfParticles)
    ...
    def returnRandomSearchSpaces(self, p):
        if len(p.blocks) == len(self.ISS):  # len(self.ISS) = numberofPraticle
            p.blocks = {}
    ...
    def searchOptimum(self, net, input, classes, j):
        ...
        while self.queries > self.numOfQueries:
            ...
            #     return self.bestPosition, self.bestFitness, iteration, self.numOfQueries, True
    ...
    def labelCheck(self):
        ...
        if self.targeted == True:
            if not 'lfw' in self.dataset and self.swarmLabel == self.classIND:
                return True
            elif 'lfw' in self.dataset:
                if self.topN == 1:
