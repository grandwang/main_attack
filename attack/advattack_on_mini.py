...
parser.add_argument('--blockSize', type=int, help='Initial blocksize for seperating image into tiles', default=2)
...
def initX(net, xs, input, classes, watermark, sl, label):
    ...
    with torch.no_grad():
        ...
        percentage = torch.nn.functional.softmax(output, dim=1)[0]  # 对最小预测值的位置进行softmax
...
def init_position_x(X1, X2, input, label, net):
    ...
    for k in range(1, 3):
        if k == 1:
            fit, X_best = initX(net, X1, input, classes, watermark, sl,
                                label)
        if k == 2:
...
def getModel_X(model_name):
    if dataset == 'imagenet224':
        model, input_size = initialize_model(model_name, num_classes=1000, feature_extract=True, use_pretrained=True)
    if dataset == 'CIFAR10':
        model, input_size = initialize_model(model_name, num_classes=10, feature_extract=True, use_pretrained=False)
...
def Initialization(pred, input, model, i, lowerBoundary, upperBoundary, j, classes, path, label):
    ...
    solu_plc_slice = init_position_SURF(numOfParticles, dim, path)
...
if __name__ == '__main__':
    ...
    watermark, watermark_x1, watermark_y1, size = Open_logo(os.path.join(
        '../logo/MIT.png'), sl, dataset)

    value_up_range = [200, input_size - watermark_x1, input_size - watermark_y1]
    value_down_range = [100, 0, 0]

    data_dir = '../datasets/imagenet-mini/train'

    traindata = ImageFolder(data_dir)
            ...
            if matchObj:
                ...
                if (pred == classes[j]):
                    ...
                    if (mmpsoattack(input, numOfParticles, dim, classes[j], path[i], i, j)):
                        print(filename[i], "attack success")
                        count1 += 1
                        print(count, "of images successfullt attack, ASR:", float(count1 / count), end='\n')
                    else:
                        print(filename[i], "attack fail", end='\n')
                        print(count, "of images successfullt attack, ASR:", float(count1 / count), end='\n')
                    end = time.time()
